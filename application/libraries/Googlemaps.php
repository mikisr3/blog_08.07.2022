...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyA1B-Ti7B-eYdEKoSwCodEhNxuMsRB0pK4'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $clusterMinimumClusterSize	= 3;						// The minimum number of markers to be in a cluster before the markers are hidden and a count is shown
	...
	var $infowindowMaxWidth			= 100;						// The maximum width of the infowindow in pixels. Expecting an integer without units
	...
	var $zoom						= 2;						// The default zoom level of the map. If set to "auto" will autozoom/center to fit in all visible markers. If "auto", also overrides the $center parameter
	...
	function add_marker($params = array())
	{
		...
		$marker['onpositionchanged'] = '';
								// JavaScript performed when the markers position changes
		...
		if ($marker['infowindow_content']!="") {
			...
			$marker['infowindow_content'] = str_replace('"', '\"', $marker['infowindow_content']); //iw_'.$this->map_name.'
			...
			google.maps.event.addListener(marker_'.$marker_id.', "mouseover", function(event) { 
			 ...
			 infoWindow = new google.maps.InfoWindow(); 
				...
				infoWindow.open('.$this->map_name.', this);
...
google.maps.event.addListener(marker_'.$marker_id.', "mouseout", function() {

				const myTime = setTimeout(function(){

					infoWindow.close('.$this->map_name.', this);
...
';
	...
	function add_circle($params = array())
	{
		...
		$circle['strokeWeight'] = '1';							// The thickness of the circle border
	...
	function create_map()
	{
		...
		if ($this->maps_loaded == 0)
		{
			if ($this->apiKey!="") 
			{
				...
				$apiLocation = 'https://maps.googleapis.com/maps/api/js?key='.$this->apiKey.'&libraries=geometry'; //.'&';
			}
			else
			{
		...
		if ($this->cluster) {
			...
			markerCluster = new MarkerClusterer('.$this->map_name.',markers_map,{imagePath: "https://developers.google.com/maps/documentation/javascript/examples/markerclusterer/m"});
			';
		}
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		$data_location = "https://maps.google.com/maps/api/geocode/json?address=".urlencode(utf8_encode($address))."&key=".$this->apiKey;
